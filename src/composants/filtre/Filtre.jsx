...
export const Filtre = ({ typeProduits, liste, setListe, setFiltre }) => {
  ...
  const handleFiltre = (e) => {
    e.target.checked
      ? setFiltres([...filtres, e.target.name])
      : setFiltres(
          filtres.filter((typeProduit) => typeProduit !== e.target.name),
        );
  };

  useEffect(() => {
    filtres.length > 0
      ? (setListe(listeFiltre()), setFiltre(true))
      : (setListe(liste), setFiltre(false));
  }, [filtres]);
